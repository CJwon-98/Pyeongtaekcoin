Pyeongtaekcoin Core version 0.18.1 is now available from:

  <https://download.pyeongtaekcoin.org/pyeongtaekcoin-0.18.1/>
...
`/Applications/Pyeongtaekcoin-Qt` (on Mac) or `pyeongtaekcoind`/`pyeongtaekcoin-qt` (on
Linux).
...
Pyeongtaekcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Pyeongtaekcoin Core on unsupported systems.
...
10.10. Additionally, Pyeongtaekcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@pyeongtaekcoin.org) the Pyeongtaekcoin Core
...
in PTC per kilobyte, not pyeongtoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/pyeongtaekcoin-conf.md)
  about the `pyeongtaekcoin.conf` file describes how to use it to configure
  Pyeongtaekcoin Core.

- A new document introduces Pyeongtaekcoin Core's BIP174 [Partially-Signed
  Pyeongtaekcoin Transactions
  (PSBT)](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Pyeongtaekcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Pyeongtaekcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Pyeongtaekcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Pyeongtaekcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/pyeongtaekcoin-project/pyeongtaekcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart pyeongtaekcoind with the `-deprecatedrpc=generate`
...
- A new `pyeongtaekcoin-wallet` tool is now distributed alongside Pyeongtaekcoin
...
This section describes planned changes to Pyeongtaekcoin Core that may affect
other Pyeongtaekcoin software and services.

- Since version 0.16.0, Pyeongtaekcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Pyeongtaekcoin Core 0.20 (expected about
  a year after 0.18), Pyeongtaekcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Pyeongtaekcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Pyeongtaekcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Pyeongtaekcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Pyeongtaekcoin Core
...
- A sub-project of Pyeongtaekcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Pyeongtaekcoin Core.  See their [project
  page](https://github.com/pyeongtaekcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Pyeongtaekcoin Core's own implementation, although entropy
  gathered by Pyeongtaekcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Pyeongtaekcoin Core a
...
- On macOS, Pyeongtaekcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `pyeongtaekcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libpyeongtaekcoin_server.a` (jnewbery)
...
- #13926 [Tools] pyeongtaekcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like pyeongtaekcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `pyeongtaekcoin-qt -version` output formatting (hebasto)
...
- #11625 Add PyeongtaekcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libpyeongtaekcoin_server` (laanwj)
...
- #15154 Configure: pyeongtaekcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/pyeongtaekcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Pyeongtaekcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libpyeongtaekcoin_server` (Mr-Leshiy)
...
- #15947 Install pyeongtaekcoin-wallet manpage (domob1812)
- #15983 build with -fstack-reuse=none (MarcoFalke)

### Tests and QA
- #15405 appveyor: Clean cache when build configuration changes (Sjors)
- #13953 Fix deprecation in pyeongtaekcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Pyeongtaekcoin Core (Christewart)
...
- #12246 Bugfix: Only run pyeongtaekcoin-tx tests when pyeongtaekcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_pyeongtaekcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_pyeongtaekcoin` (MarcoFalke)
...
- #14186 pyeongtaekcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_pyeongtaekcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update pyeongtaekcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/pyeongtaekcoin-conf.md (hebasto)
...
- #15354 Add missing `pyeongtaekcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct pyeongtaekcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support pyeongtaekcoin folder on OSX (JimmyMow)
