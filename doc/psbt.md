# PSBT Howto for Pyeongtaekcoin Core

Since Pyeongtaekcoin Core 0.17, an RPC interface exists for Partially Signed Pyeongtaekcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/pyeongtaekcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Pyeongtaekcoin transactions that are not fully signed
...
#### Multisig with multiple Pyeongtaekcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Pyeongtaekcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `pyeongtaekcoin-cli` is used.
...
Later, when *V* PTC has been received on *Amulti*, and Bob and Carol want to
